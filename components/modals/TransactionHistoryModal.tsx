    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Transaction Archive" size="lg">
            <div className="flex flex-col h-[600px] -m-8 relative overflow-hidden bg-surface-main">
                ...
                <div className="p-6 border-b border-white/5 bg-surface-main/70 backdrop-blur-xl shrink-0 space-y-4 relative z-10">
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase text-text-muted tracking-widest opacity-80">
                    ...
                    <div className="relative group">
                         ...
                         <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full bg-surface-alt/50 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold text-text-primary outline-none focus:bg-surface-main focus:border-accent-primary/50 transition-all placeholder:text-text-muted/50 shadow-inner"
                                autoFocus
                            />
                            {searchQuery && (
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 relative z-10">
                    {secureHistory.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-text-muted opacity-40">
                            <div className="w-24 h-24 bg-surface-alt rounded-full flex items-center justify-center mb-6 border border-white/10">
                        ...
                        <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-6">
                            {secureHistory.map((group, idx) => (
                                <div key={idx} className="group/card relative overflow-hidden rounded-[2rem] border border-white/5 bg-surface-main/40 hover:bg-surface-main/60 transition-all duration-500 hover:shadow-2xl hover:border-accent-primary/20">
                                    ...
                                    {/* Customer Header */}
                                    <div className="p-5 border-b border-white/5 flex justify-between items-center bg-surface-alt/10 relative z-10">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 flex items-center justify-center font-black text-lg text-indigo-500">
                                                {group.identity.customer.charAt(0)}
                                            </div>
                                            <div>
                                                <h4 className="text-lg font-black text-text-primary uppercase tracking-tight">{group.identity.customer}</h4>
                                                <div className="flex items-center gap-3 mt-1">
                                                    <span className="text-[10px] font-mono font-bold text-text-muted flex items-center gap-1 bg-surface-alt/50 px-2 py-0.5 rounded border border-white/5">
                                                        <EyeOff size={10} /> {maskPhone(group.identity.phone)}
                                                    </span>
                                                    {group.history.length > 1 && (
                                                        <span className="text-[10px] font-bold text-accent-primary flex items-center gap-1">
                                                            <Activity size={10}/> {group.history.length} Matches
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {onSelect && (
                                            <Button 
                                                ...
                                                variant="secondary"
                                                className="h-9 text-[10px] uppercase font-black tracking-wider bg-surface-main/50 hover:bg-surface-alt border-white/10"
                                            >
                                                Select Profile
                                            </Button>
                                        )}
                                        ...
                                        <div className="space-y-4 relative z-10">
                                            {group.history.map((sale, _i) => (
                                                <div key={sale.id} className="flex gap-4 group/row">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center bg-surface-main shrink-0 mt-1 z-10 ${sale.status === 'Approved' ? 'border-emerald-500 text-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'border-amber-500 text-amber-500'}`}>
                                                        <div className={`w-2 h-2 rounded-full ${sale.status === 'Approved' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                                                    </div>
                                                    
                                                    <div className="flex-1 bg-surface-alt/20 hover:bg-surface-alt/50 border border-white/5 rounded-2xl p-3 transition-all flex justify-between items-center group-hover/row:border-accent-primary/20 backdrop-blur-sm">
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <span className="text-[10px] font-black uppercase text-text-secondary">{formatRelativeTime(sale.timestamp)}</span>
                                                                <span className="text-[10px] text-text-muted opacity-50">•</span>
                                                                <span className="text-[10px] text-text-muted font-mono">{new Date(sale.timestamp).toLocaleDateString()}</span>
                                                            </div>
                                                        ...
                                                        {onSelect && (
                                                            <button 
                                                                ...
                                                                className="flex flex-col items-center gap-1 text-[9px] font-bold text-text-muted hover:text-accent-primary uppercase tracking-wider group/clone p-2 hover:bg-surface-main/50 rounded-xl transition-all"
