...
export const CallHistory: React.FC<CallHistoryProps> = ({ logs, currentUser }) => {
    ...
    return (
        <Card variant="panel" className="flex flex-col h-full bg-surface-main overflow-hidden p-0 relative border-white/5">
            <div className="p-4 border-b border-border-subtle bg-surface-alt/50 flex justify-between items-center backdrop-blur-md sticky top-0 z-10">
                <div>
                    <h3 className="text-base font-black uppercase tracking-tight text-text-primary flex items-center gap-2">
                        <History size={16} className="text-accent-primary"/>
                        Communication Log
                    </h3>
                    <p className="text-[9px] font-bold text-text-muted uppercase tracking-widest mt-0.5">Call Logs & Durations</p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-0 bg-surface-main">
                {sortedLogs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-text-muted opacity-50 py-20">
                        ...
                        <p className="text-sm font-bold uppercase tracking-widest">No call history recorded</p>
                    </div>
                ) : (
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-surface-alt/80 backdrop-blur-sm text-[9px] font-black text-text-muted uppercase tracking-widest sticky top-0 z-10 border-b border-border-subtle shadow-sm">
                            <tr>
                                ...
                                <tr key={log.id} className="hover:bg-surface-highlight/30 transition-all group">
                                    <td className="p-3 pl-6 whitespace-nowrap font-mono text-text-muted">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] opacity-70">{new Date(log.startTime).toLocaleDateString()}</span>
                                        </div>
                                    </td>
                                    <td className="p-3">
                                        <div className="flex items-center gap-3">
                                            <div className="w-6 h-6 rounded-lg bg-surface-alt flex items-center justify-center text-[10px] font-bold text-text-secondary border border-border-subtle">
                                                {log.partnerName.charAt(0)}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="font-bold text-text-primary">{log.partnerName}</span>
                                                <span className="text-[9px] text-text-muted uppercase font-mono tracking-wide">ID::{log.partnerId.slice(-6)}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-3">
                                        <span className={`flex items-center gap-1.5 font-bold uppercase text-[10px] ${log.type === 'video' ? 'text-purple-500' : 'text-blue-500'}`}>
                                    ...
                                    <td className="p-3 text-right pr-6">
                                        <div className={`flex items-center justify-end gap-2 font-black uppercase tracking-tight text-[10px] ${getOutcomeStyles(log.outcome)}`}>
                                            {log.outcome === 'Connected' ? <CheckCircle2 size={12}/> : <XCircle size={12}/>}
                                            {log.outcome}
                                        </div>
                                    </td>
                                </tr>
