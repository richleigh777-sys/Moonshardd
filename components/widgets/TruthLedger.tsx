    ...
    return (
        <Card variant="panel" className="flex flex-col h-full overflow-hidden border-border-subtle bg-surface-main shadow-2xl rounded-[2rem]">
            <div className="p-6 border-b border-border-subtle bg-surface-alt/40 flex justify-between items-center backdrop-blur-md sticky top-0 z-20">
                <div>
                    <h3 className="text-lg font-bold text-text-primary uppercase tracking-tight flex items-center gap-3">
                        <div className="p-2 bg-accent-primary/10 rounded-xl text-accent-primary border border-accent-primary/20">
                            <FileText size={20} />
                        </div>
                        Audit Log
                    </h3>
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-1 ml-1">System Events • Confidential</p>
                </div>
                <div className="px-3 py-1 bg-surface-main rounded-full border border-border-subtle shadow-sm flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-mono text-text-muted uppercase">SECURE_CONNECTION</span>
                </div>
            </div>
            
            <div className="flex-1 overflow-y-auto p-0 font-mono text-xs custom-scrollbar bg-surface-main/50 relative z-10">
                <table className="w-full text-left">
                    <thead className="sticky top-0 bg-surface-main/95 backdrop-blur-xl text-text-muted font-bold uppercase text-[9px] tracking-widest border-b border-border-subtle z-10 shadow-sm">
                        <tr>
                        ...
                        {sortedLogs.length === 0 ? (
                            ...
                            <tr key={log.id} className="hover:bg-surface-alt/30 transition-colors group">
                                ...
                                <td className="p-4">
                                    ...
                                    <p className="text-[8px] text-text-muted uppercase tracking-wider opacity-60">ID: {log.agentId}</p>
                                </td>
                                <td className="p-4">
                                    <span className={`px-2 py-1 rounded text-[9px] font-bold border uppercase tracking-wider ${
                                        log.module === 'SALE' ? 'border-emerald-500/30 text-emerald-600 bg-emerald-500/5' : 
                                ...
                                <td className="p-4 text-right pr-6 text-text-muted">
                                    <p className="text-[10px] leading-relaxed truncate max-w-[300px] inline-block" title={log.details}>
                                        {log.details}
                                    </p>
                                    <p className="text-[8px] font-mono opacity-40 mt-0.5">REF: {log.id.slice(-6)}</p>
                                </td>
                            </tr>
                        ))}
                    </tbody>
