        ...
        <Card variant="panel" className="flex flex-col h-full border-border-subtle p-0 relative group bg-surface-main overflow-hidden shadow-inner">
            <div className="p-3 border-b border-border-subtle bg-surface-alt/50 flex items-center justify-between shrink-0 backdrop-blur-md relative z-10">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-text-muted uppercase tracking-[0.15em] font-bold text-[10px]">Community Log</span>
                </div>
                <div className="flex gap-1.5 items-center">
                    <div className="text-[8px] font-bold text-emerald-500 uppercase tracking-widest mr-1">Live Sync</div>
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_5px_#10B981]"></div>
                </div>
            </div>

            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar scroll-smooth relative z-10 font-sans text-[10px]">
                {displayLogs.map((log) => (
                    <div key={log.id} className="flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-500 group/row">
                        <span className="text-text-muted opacity-40 shrink-0 font-mono text-[9px]">[{new Date(log.timestamp).toLocaleTimeString([], {hour12: false, hour: '2-digit', minute:'2-digit'})}]</span>
                ...
                {displayLogs.length === 0 && <span className="text-text-muted opacity-20 animate-pulse uppercase tracking-widest text-[9px] block text-center py-10">Listening for activity...</span>}
            </div>
        </Card>
    );
};
