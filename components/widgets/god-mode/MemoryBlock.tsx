        ...
        >
            <div className="flex justify-between items-start relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h4 className={`text-xs font-[700]  tracking-wider ${active ? 'text-accent-primary' : 'text-text-primary'}`}>{label}</h4>
                        <p className="text-xs font-mono text-text-muted mt-0.5">{(size / 1024).toFixed(2)} KB</p>
                    </div>
                </div>
                <div className="flex flex-col items-end">
                    <span className={`text-xs font-[700]  tracking-widest ${integrity > 90 ? 'text-status-success' : 'text-status-warning'}`}>
                        {integrity}% Health
                    </span>
