        ...
        <div className="h-full flex flex-col animate-in fade-in duration-500">
            ...
            <Card variant="panel" className="p-6 mb-6 bg-surface-main border-border-subtle shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-text-primary uppercase tracking-tight">Financial Ledger</h2>
                    ...
                    history.map((cycle) => (
                        <div 
                            ...
                            className={`
                                ...
                                ${cycle.status === 'Open' ? 'bg-surface-main border-accent-primary/30 shadow-lg shadow-accent-primary/5' : 'bg-surface-main border-border-subtle hover:border-emerald-500/30'}
                            `}
                        >
                            {/* Background Deco */}
                            ...
                            <div className="relative z-10 space-y-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border mb-2 ${getStatusStyle(cycle.status)}`}>
                                            {getStatusIcon(cycle.status)} {cycle.status}
                                        </div>
                                        <h3 className="text-lg font-black text-text-primary uppercase tracking-tight">{cycle.label}</h3>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center p-3 bg-surface-alt/50 rounded-xl border border-border-subtle">
                                        <span className="text-[10px] font-bold text-text-secondary uppercase tracking-wider">Sales Volume</span>
                                        <span className="text-sm font-black text-text-primary num-font">${cycle.volume.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between items-center p-3 bg-surface-alt/50 rounded-xl border border-border-subtle">
                                        <span className="text-[10px] font-bold text-text-secondary uppercase tracking-wider">Deals Closed</span>
                                        <span className="text-sm font-black text-text-primary num-font">{cycle.salesCount}</span>
                                    </div>
                                </div>

                                <div className="pt-6 border-t border-border-subtle/50 flex justify-between items-end">
                                    <div>
                                        <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1">Net Payout</p>
                                        <p className="text-3xl font-black text-text-primary num-font tracking-tighter">${cycle.netPayout.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                                    </div>
