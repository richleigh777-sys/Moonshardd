...
export const PayslipModal: React.FC<PayslipModalProps> = ({ cycle, onClose, currentUser }) => {
    ...
    const getStatusConfig = (status: string) => {
        if (status === 'Paid') return { color: 'text-status-success', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', icon: CheckCircle2 };
        if (status === 'Processing') return { color: 'text-status-warning', bg: 'bg-amber-500/10', border: 'border-amber-500/20', icon: Clock };
        ...
        <Modal isOpen={!!cycle} onClose={onClose} title="Statement of Earnings" size="2xl">
            <div className="flex flex-col h-full -m-8 bg-[#09090b] text-white">
                        ...
                        <div className="flex gap-5">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4 mt-2 text-xs font-medium text-text-muted">
                                    ...
                                    <span className="w-px h-3 bg-surface-highlight"></span>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-border-subtle">
                    {/* Net Pay Card */}
                    <div className="md:col-span-2 relative overflow-hidden rounded-3xl bg-surface-main border border-status-success/30 p-6 flex flex-col justify-between group shadow-[0_0_30px_rgba(16,185,129,0.05)]">
                        ...
                        <div className="mt-6 flex items-center gap-2 text-xs font-bold text-text-muted  tracking-wider relative z-10">
                            <span className="bg-surface-alt px-3 py-1.5 rounded border border-border-subtle text-status-success">Verified</span>
                            <span>•</span>
                ...
                <div className="grid grid-cols-4 divide-x divide-white/5 border-b border-border-subtle bg-surface-main/50">
                    ...
                    <div className="p-4 flex flex-col items-center justify-center text-center group hover:bg-surface-highlight transition-colors">
                        ...
                        <span className="text-lg font-bold text-status-warning">+${cycle.spiffs.toLocaleString()}</span>
                    </div>
                    <div className="p-4 flex flex-col items-center justify-center text-center group hover:bg-surface-highlight transition-colors">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-[#09090b] z-10 text-xs font-[700]  text-text-muted tracking-widest border-b border-border-subtle shadow-sm">
                            <tr>
                                ...
                                <th className="p-4 text-right text-status-warning/80">Bonus</th>
                                <th className="p-4 text-right text-rose-400/80">Ded.</th>
                                <th className="p-4 text-right pr-8 text-status-success/80">Net</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5 text-xs font-medium">
                            {cycle.sales.map((item) => (
                                <tr key={item.sale.id} className="hover:bg-surface-highlight transition-colors group">
                                    ...
                                    <td className="p-4 text-right font-mono text-status-warning group-hover:text-status-warning">
                                    ...
                                    <td className="p-4 pr-8 text-right font-[700] font-mono text-status-success group-hover:text-status-success">
