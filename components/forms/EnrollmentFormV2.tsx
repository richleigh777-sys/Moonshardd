...
export default function EnrollmentFormV2({ onSuccess, onCancel, prefillPhone }: any) {
    ...
    if (mode === 'approved' && lastOrder) {
        return (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="absolute inset-0 z-50 flex items-center justify-center bg-[#0A0A0A]/90 p-4 font-sans select-none backdrop-blur-sm rounded-xl">
                 <div className="bg-[#141414] border border-[#8BA888]/30 rounded-3xl p-12 max-w-lg w-full text-center shadow-2xl space-y-6">
                     ...
                     <div className="bg-[#1A1A1A] rounded-2xl p-6 font-mono text-sm space-y-3 border border-white/5 text-left mb-8">
                     ...
                     <button onClick={onSuccess} className="w-full py-4 bg-gradient-to-r from-[#E6C280] to-[#C4A470] text-black font-bold uppercase tracking-widest rounded-xl hover:shadow-[0_0_30px_rgba(196,164,112,0.3)] transition-all">Return to Dashboard</button>
                 </div>
            </motion.div>
        );
    }
    ...
    return (
        <div className="flex flex-col bg-[#0A0A0A] font-sans select-none rounded-2xl border border-white/5 relative isolate">
            <style>{`
            ...
            <header className="sticky top-0 z-[100] h-20 border-b border-white/5 flex items-center justify-between px-10 shrink-0 bg-[#141414]/90 backdrop-blur-md">
                <div className="flex items-center gap-6">
                    <div className="text-[#C4A470] font-bold text-xl tracking-widest uppercase">Secure Enrollment</div>
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => { handleClear(); }} className="px-5 py-2.5 border border-rose-500/50 bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 hover:border-rose-500 rounded-xl font-bold tracking-wider transition-all uppercase text-sm shadow-[0_0_15px_rgba(244,63,94,0.3)]" title="Clear or Reset Form Data">
                        Clear Form
                    </button>
                    <button onClick={onCancel} className="px-5 py-2.5 border border-[#C4A470]/50 bg-[#C4A470]/20 text-[#C4A470] hover:bg-[#C4A470]/30 hover:border-[#C4A470] rounded-xl font-bold tracking-wider transition-all uppercase text-sm shadow-[0_0_15px_rgba(196,164,112,0.3)]">
                        Cancel & Return
                    </button>
                </div>
            </header>
            ...
            <div className="flex flex-col items-center">
                ...
                <div className="w-full shrink-0 min-h-[800px] relative isolate">
                    {error && (
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-[#1A1A1A] border border-rose-500/50 text-rose-400 px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3">
