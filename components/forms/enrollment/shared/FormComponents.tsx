...
export const FormLabel = ({ icon: Icon, children, className = "" }: FormLabelProps) => (
    <label className={`text-[8px] font-black uppercase text-text-muted/60 tracking-[0.2em] mb-1 flex items-center gap-1.5 ml-1 transition-colors select-none ${className}`}>
        {Icon && <Icon size={10} className="text-accent-primary/60" strokeWidth={3} />}
        {children}
    </label>
);
...
export const FormInput = ({ icon: Icon, rightElement, status = 'default', className = "", ...props }: FormInputProps) => {
    ...
    return (
        <div className="relative group w-full">
            <div className={`absolute -inset-[1px] rounded-lg opacity-0 group-focus-within:opacity-100 transition-opacity duration-700 pointer-events-none z-0 bg-gradient-to-br ${
            ...
            <div className={`
                relative z-10 w-full rounded-lg border-none backdrop-blur-3xl transition-all duration-500
                ${isError ? 'bg-rose-500/[0.03]' : isValid ? 'bg-emerald-500/[0.03]' : 'bg-white/[0.03] group-hover:bg-white/[0.05] group-focus-within:bg-black/60'}
            ...
            `}>
                {Icon && (
                    <div className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors duration-300 pointer-events-none z-20 ${
                        isError ? 'text-rose-400' : isValid ? 'text-emerald-400' : 'text-text-muted group-focus-within:text-accent-primary'
                    }`}>
                ...
                {(isError || isValid) && !rightElement && (
                    <div className={`absolute right-3 top-1/2 -translate-y-1/2 z-20 animate-in zoom-in duration-300 ${isError ? 'text-rose-500' : 'text-emerald-500'}`}>
                        {isError ? <AlertCircle size={14} strokeWidth={3} /> : <CheckCircle size={14} strokeWidth={3} />}
                    </div>
                )}
            </div>
        </div>
...
export const FormSelect = ({ icon: Icon, children, className = "", ...props }: any) => (
    <div className="relative group w-full">
        <div className="absolute -inset-[1px] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none z-0 bg-gradient-to-br from-white/10 via-white/5 to-transparent"></div>
        <div className="relative z-10 w-full rounded-lg bg-white/[0.03] group-hover:bg-white/[0.05] ring-1 ring-white/10 group-focus-within:ring-accent-primary/40 shadow-[inset_0_1px_6px_rgba(0,0,0,0.4)] backdrop-blur-3xl transition-all duration-500">
            {Icon && (
