...
const CleanProductSelector = ({ products = [], presets = [], cart = [], onAdd, onRemove }: any) => {
    return (
        <div className="space-y-12">
            <div>
                <h4 className="text-[#A0A0A0] text-sm uppercase tracking-widest font-semibold mb-6">Core Treatments</h4>
                <div className="grid grid-cols-2 gap-4">
                    {products.map((p: any) => {
                        const count = cart.filter((c:any)=>c.product===p.name).length;
                        return (
                            <div key={p.id} className="p-5 flex items-center justify-between rounded-2xl bg-[#1A1A1A] border border-white/5 hover:border-[#C4A470]/50 transition-all cursor-default">
                                <div>
            ...
            {presets?.length > 0 && (
                <div>
                    <h4 className="text-[#A0A0A0] text-sm uppercase tracking-widest font-semibold mb-6">Care Bundles</h4>
                    <div className="grid grid-cols-2 gap-4">
                         {presets.map((preset: any) => (
                             <div key={preset.id} className="p-5 rounded-2xl bg-[#141414] border border-[#C4A470]/20 flex items-center justify-between">
                                 <div>
                                 ...
                                 }} className="px-5 py-2.5 rounded-full bg-[#C4A470] text-black font-bold text-sm tracking-wide uppercase flex items-center gap-2 hover:shadow-[0_0_20px_rgba(196,164,112,0.4)] transition-all">Add Bundle</button>
                             </div>
                         ))}
                    </div>
                </div>
    ...
    return (
        <div className="w-full h-full flex items-start justify-center p-8 overflow-y-auto custom-scrollbar">
            <div className="w-full max-w-[1400px] flex flex-col md:flex-row gap-12 pb-24">
                 ...
                 <div className="w-full md:w-[65%] flex flex-col bg-[#141414] border border-white/5 rounded-[24px] shadow-2xl overflow-hidden min-h-[600px] h-fit">
                 ...
                 <div className="w-full md:w-[35%] flex flex-col gap-8 h-fit">
                     ...
                     <div className="bg-[#1A1A1A] border border-white/5 rounded-[24px] p-6 shadow-xl">
                          <h3 className="text-[#C4A470] font-semibold text-sm tracking-widest uppercase flex items-center gap-2 mb-4">
                     ...
                     <div className="flex-1 bg-[#141414] border border-white/5 rounded-[24px] p-8 shadow-2xl flex flex-col min-h-[350px]">
