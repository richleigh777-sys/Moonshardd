        ...
        <Modal isOpen={isOpen} onClose={onClose} title={formData.id ? "Configure SKU" : "Initialize New SKU"} size="lg">
            <div className="flex flex-col h-[600px] -m-8">
                ...
                <div className="flex items-center px-8 border-b border-border-subtle bg-surface-alt/30 shrink-0">
                    <button onClick={() => setActiveTab('basics')} className={`py-4 px-4 text-sm font-[700]  tracking-widest border-b-2 transition-all flex items-center gap-2 ${activeTab === 'basics' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-text-muted hover:text-text-primary'}`}><Package size={16}/> Identity</button>
                    <button onClick={() => setActiveTab('inventory')} className={`py-4 px-4 text-sm font-[700]  tracking-widest border-b-2 transition-all flex items-center gap-2 ${activeTab === 'inventory' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-text-muted hover:text-text-primary'}`}><BarChart3 size={16}/> Economics</button>
                    <button onClick={() => setActiveTab('variants')} className={`py-4 px-4 text-sm font-[700]  tracking-widest border-b-2 transition-all flex items-center gap-2 ${activeTab === 'variants' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-text-muted hover:text-text-primary'}`}><Layers size={16}/> Variants</button>
                </div>
                ...
                <div className="p-4 border-t border-border-subtle bg-surface-main flex justify-end gap-3 shrink-0">
                    ...
                    <Button variant="primary" onClick={handleSave} className="h-12 px-8 shadow-lg shadow-accent-primary/20 bg-gradient-to-r from-accent-primary to-indigo-600 border border-border-subtle text-sm font-[700]  tracking-widest hover:brightness-110">
