...
const BountyCard: React.FC<{ rule: SpiffRule, onUpdate: (r: Partial<SpiffRule>) => void, onDelete: () => void }> = ({ rule, onUpdate, onDelete }) => (
    <div className="relative p-5 bg-surface-main border border-border-subtle rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden hover:border-status-warning/30">
        <div className="absolute top-0 right-0 p-8 opacity-[0.03] pointer-events-none group-hover:opacity-10 transition-opacity group-hover:scale-110 duration-700">
            <Award size={120} className="text-status-warning" />
        </div>
        ...
        <div className="space-y-4 relative z-10">
            <div className="flex justify-between items-start">
                <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                    ...
                    className="bg-transparent text-sm font-[700]  text-text-primary outline-none focus:text-status-warning w-full placeholder:text-text-muted/30 tracking-tight"
                    placeholder="BOUNTY NAME"
                />
                <button onClick={() => { sfx.playDecline(); onDelete(); }} className="text-text-muted hover:text-status-error transition-colors p-1.5 hover:bg-surface-alt rounded-lg"><Trash2 size={16}/></button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-surface-alt/50 p-3 rounded-2xl border border-border-subtle group-focus-within:border-accent-primary/50 transition-colors">
                    <label className="text-sm font-bold text-text-muted  block mb-1">Min Sale Amount</label>
                    <div className="flex items-center gap-1">
                        ...
                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                            ...
                            className="w-full bg-transparent text-base font-[700] num-font outline-none text-text-primary" 
                        />
                    </div>
                </div>
                <div className="bg-amber-500/10 p-3 rounded-2xl border border-amber-500/20">
                    <label className="text-sm font-bold text-amber-600  block mb-1">Bonus Payout</label>
                    <div className="flex items-center gap-1 text-status-warning">
                        ...
                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                            ...
                            className="w-full bg-transparent text-base font-[700] num-font outline-none" 
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-border-subtle/50">
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-surface-alt rounded-lg">
                    <Clock size={16} className="text-text-muted"/>
                    <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                        type="number" 
                        value={rule.minHours} 
                        onChange={e => onUpdate({ minHours: parseInt(e.target.value) })} 
                        className="w-8 bg-transparent text-xs font-[700] text-text-primary outline-none text-center" 
                    />
                    <span className="text-xs font-bold text-text-muted ">Hrs/Day</span>
                </div>
                <div className="h-px flex-1 bg-border-subtle/50"></div>
                <span className="text-sm font-[700]  text-status-success bg-emerald-500/10 px-2.5 py-1 rounded">Active</span>
            </div>
            <p className="text-xs text-text-muted font-medium opacity-60 text-center italic">Applied per qualifying sale if daily hours met.</p>
        </div>
    </div>
);
...
export const FinancialsTab: React.FC<FinancialsTabProps> = ({ config, onChange }) => {
    ...
    return (
        <section className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
            <SectionHeader icon={DollarSign} title="Compensation Architecture" sub="Commission Rates & Automated Bounties" color="text-status-success" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-1 bg-surface-alt/30 rounded-[2.5rem] border border-border-subtle">
                {[
                    ...
                    <div key={idx} className={`p-6 bg-surface-main rounded-[2rem] border border-border-subtle shadow-sm group hover:border-${item.color}-500/30 transition-all relative overflow-hidden`}>
                        ...
                        <label className="text-xs font-[700] text-text-muted  tracking-widest mb-3 flex items-center gap-1.5 relative z-10">
                        ...
                        <div className="flex items-center gap-1 relative z-10">
                            ...
                            <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                                ...
                                className={`w-full bg-transparent text-4xl font-[700] text-text-primary num-font outline-none focus:text-${item.color}-500 transition-colors`} 
                            />
                            {item.unit !== '$' && <span className="text-xs font-bold text-text-muted  self-end mb-2">{item.unit}</span>}
                        </div>
                        {item.key === 'shippingDeduction' && (
                            <p className="relative z-10 mt-2 text-xs text-text-muted font-bold opacity-70">
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-amber-500/10 rounded-xl text-status-warning border border-amber-500/20">
                            <Star size={18}/>
                        </div>
                        <div>
                            <h5 className="text-sm font-[700] text-text-primary  tracking-widest">Incentive Matrix</h5>
                            <p className="text-xs text-text-muted font-bold">Automated Spiff Distribution</p>
                        </div>
                    </div>
                    <Button onClick={addSpiff} variant="primary" className="h-10 text-xs  font-[700] tracking-widest px-6 shadow-lg shadow-accent-primary/20">
                        <Plus size={16} strokeWidth={4} className="mr-2"/> Create Bounty
                    </Button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    {(!config.spiffRules || config.spiffRules.length === 0) && (
                        <div className="col-span-full py-12 flex flex-col items-center justify-center text-text-muted opacity-40 border-2 border-dashed border-border-subtle rounded-3xl">
                            ...
                            <p className="text-xs font-[700]  tracking-widest">No active incentives</p>
                        </div>
                    )}
                </div>
            </div>
