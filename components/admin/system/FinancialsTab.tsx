        ...
        <div className="space-y-5 relative z-10">
            <div className="flex justify-between items-start">
                <Input 
                    ...
                    className="bg-transparent h-10 border-none px-0 text-sm font-black text-text-primary focus:text-status-warning w-full placeholder:text-text-muted/30 tracking-wider uppercase ring-0 shadow-none focus:ring-0"
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-surface-alt/50 p-3.5 rounded-xl border border-border-subtle group-focus-within:border-accent-primary/50 transition-colors shadow-inner">
                    <label className="text-[10px] uppercase font-black text-text-muted tracking-widest block mb-1">Min Threshold</label>
                    <div className="flex items-center gap-1.5">
                        ...
                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                            ...
                            className="w-full bg-transparent text-lg font-mono font-black outline-none text-text-primary" 
                        />
                    </div>
                </div>
                <div className="bg-status-warning/10 p-3.5 rounded-xl border border-status-warning/20 shadow-inner">
                    <label className="text-[10px] uppercase font-black text-status-warning/80 tracking-widest block mb-1">Yield Payout</label>
                    <div className="flex items-center gap-1.5 text-status-warning">
                        ...
                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                            ...
                            className="w-full bg-transparent text-lg font-mono font-black outline-none" 
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border-subtle/50">
                <div className="flex items-center gap-2 px-3 py-2 bg-surface-alt rounded-lg border border-border-subtle">
                    ...
                    <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                        ...
                        className="w-10 bg-transparent text-xs font-mono font-black text-text-primary outline-none text-center" 
                    />
                    <span className="text-[10px] font-black uppercase text-text-muted tracking-widest">Hrs/Day</span>
                </div>
                <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-status-success animate-pulse shadow-[0_0_8px_rgba(var(--color-status-success),0.8)]"></span>
                    <span className="text-[10px] font-black text-status-success tracking-widest uppercase">Active</span>
                </div>
            </div>
            <p className="text-[10px] text-text-muted font-bold opacity-60 text-center uppercase tracking-widest leading-relaxed">
        ...
        >
                    ...
                    {metricCards.map((item, idx) => (
                        <Card key={idx} className={`p-6 sm:p-8 bg-surface-main relative overflow-hidden shadow-2xl transition-all duration-300 group ${item.hoverClass}`}>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                <h4 className="text-[10px] font-black text-text-primary tracking-widest uppercase flex items-center gap-2 mb-6">
                                ...
                                <div className="space-y-4 pt-2">
                                    <div className="flex items-center gap-2">
                                        {item.unit === '$' && <span className={`text-2xl font-black ${item.colorClass}`}>$</span>}
                                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                                            type="number" 
                                            value={item.value} 
                                            onChange={e => onChange(item.key as any, parseInt(e.target.value) || 0)} 
                                            className={`w-full bg-transparent text-4xl lg:text-5xl font-black font-mono text-text-primary outline-none transition-colors border-b-2 border-transparent focus:border-text-primary/20 pb-1`} 
                                        />
                                        {item.unit !== '$' && <span className={`text-xl font-black ${item.colorClass} self-end mb-2`}>{item.unit}</span>}
                                    </div>
                                    {item.key === 'shippingDeduction' && (
                                        <p className="text-[10px] uppercase font-bold text-text-muted tracking-widest leading-relaxed">
            ...
            <motion.div variants={itemVariants} className="pt-8 border-t border-border-subtle space-y-6">
                <div className="flex items-center gap-4 px-2">
                    <div className="p-3 bg-surface-alt rounded-2xl text-text-primary border border-border-subtle shadow-inner">
                        <Database size={24}/>
                    </div>
                    <div>
                        <h3 className="text-lg font-black text-text-primary uppercase tracking-tighter">Payroll Parameters</h3>
                        <p className="text-sm text-text-muted font-bold">Define global settlement intervals & thresholds.</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-surface-main border border-border-subtle rounded-2xl p-5 shadow-sm focus-within:border-accent-primary/50 transition-colors">
                        <label className="text-[10px] font-black uppercase text-text-muted tracking-widest block mb-3">Commission Architecture</label>
                        <select 
                    ...
                    <div className="bg-surface-main border border-border-subtle rounded-2xl p-5 shadow-sm focus-within:border-accent-primary/50 transition-colors">
                        <label className="text-[10px] font-black uppercase text-text-muted tracking-widest block mb-3">Ledger Sync Cycle</label>
                        <select 
                    ...
                    <div className="bg-surface-main border border-border-subtle rounded-2xl p-5 shadow-sm focus-within:border-accent-primary/50 transition-colors">
                        <label className="text-[10px] font-black uppercase text-text-muted tracking-widest block mb-3">Min Release Threshold ($)</label>
                        <div className="flex items-center gap-2 bg-surface-alt border border-border-subtle rounded-xl px-4 focus-within:border-accent-primary">
                            ...
                            <input 
                                ...
                                className="w-full bg-transparent py-3 text-sm font-mono font-black text-text-primary outline-none"
                            />
                        </div>
                    </div>
                </div>
            ...
            <motion.div variants={itemVariants} className="pt-8 border-t border-border-subtle space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-status-warning/10 rounded-2xl text-status-warning border border-status-warning/20 shadow-inner">
                            <Star size={24}/>
                        </div>
                        <div>
                            <h3 className="text-lg font-black text-text-primary uppercase tracking-tighter">Incentive Matrix</h3>
                            <p className="text-sm text-text-muted font-bold">Automated algorithmic spiff deployment rules.</p>
                        </div>
                    </div>
                    <button 
                        onClick={addSpiff}
                        className="shrink-0 px-5 py-3 bg-status-warning text-surface-main font-black text-xs uppercase tracking-widest rounded-xl hover:bg-status-warning/90 transition-all shadow-[0_0_20px_rgba(var(--color-status-warning),0.4)] hover:shadow-[0_0_30px_rgba(var(--color-status-warning),0.6)] flex items-center gap-2"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    {(!config.spiffRules || config.spiffRules.length === 0) && (
                        <Card className="col-span-full py-16 flex flex-col items-center justify-center text-text-muted bg-surface-alt/30 border-2 border-dashed border-border-subtle">
                            ...
                            <p className="text-xs font-black uppercase tracking-widest text-text-primary mb-1">No Active Protocols</p>
            ...
            <motion.div variants={itemVariants} className="pt-8 border-t border-border-subtle">
                <Card className="p-8 bg-surface-main border border-border-subtle shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-8">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-status-success/10 text-status-success rounded-2xl border border-status-success/20 shadow-inner">
                                <ShieldCheck size={24} />
                            </div>
                            <div>
                                <h3 className="text-lg font-black text-text-primary uppercase tracking-tighter">Executive Forecasting Sandbox</h3>
...
const ProjForecast: React.FC<ForecastProps> = ({ config }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-surface-alt/50 p-6 sm:p-8 rounded-[2rem] border border-border-subtle shadow-inner">
            <div className="lg:col-span-5 space-y-6">
                <div className="space-y-3">
                    ...
                    <div className="relative group">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-black text-text-muted group-focus-within:text-status-success transition-colors">$</span>
                        <input autoComplete="off" data-lpignore="true" data-prevent-autofill="true" spellCheck={false} 
                            type="number" 
                            value={projectedSales || ''}
                            onChange={e => setProjectedSales(parseInt(e.target.value) || 0)}
                            className="w-full bg-surface-main border border-border-strong rounded-xl py-3.5 pl-8 pr-4 text-sm font-mono font-black text-text-primary outline-none focus:border-status-success focus:ring-1 focus:ring-status-success/30 transition-all shadow-sm"
                        />
                    </div>
                </div>

                <div className="space-y-3">
                    <div className="flex justify-between items-center text-[10px] font-black text-text-primary uppercase tracking-widest">
                        <div className="flex items-center gap-2">
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-5 bg-surface-main border border-border-subtle rounded-2xl flex flex-col justify-between items-center text-center relative overflow-hidden group">
                    ...
                    <span className="text-[10px] font-black text-text-muted uppercase tracking-widest relative z-10 mb-4">Gross Obligation</span>
                    <span className="text-2xl font-black font-mono text-text-primary relative z-10 break-all w-full leading-none">${baseCommAmt.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                    <span className="text-[10px] uppercase font-bold text-text-muted tracking-widest mt-4 relative z-10">Pre-liability basis</span>
                </div>
                <div className="p-5 bg-status-error/5 border border-status-error/20 rounded-2xl shadow-inner flex flex-col justify-between items-center text-center">
                    <span className="text-[10px] font-black text-status-error uppercase tracking-widest mb-4">Clawback Lock</span>
                    <span className="text-2xl font-black font-mono text-status-error break-all w-full leading-none">${clawbackReserve.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                    <span className="text-[10px] uppercase font-bold text-status-error/70 tracking-widest mt-4">{config.clawbackWindow || 90}d frozen asset</span>
                </div>
                <div className="p-5 bg-status-success/10 border border-status-success/30 rounded-2xl shadow-inner flex flex-col justify-between items-center text-center relative">
                    <div className="absolute -top-10 -right-10 w-24 h-24 bg-status-success/20 rounded-full blur-xl"></div>
                    <span className="text-[10px] font-black text-status-success uppercase tracking-widest mb-4 relative z-10">Estimated Settle</span>
                    <span className="text-2xl font-black font-mono text-status-success relative z-10 break-all w-full leading-none">${estimatedSettlePayout.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                    <span className="text-[10px] uppercase font-bold text-status-success/70 tracking-widest mt-4 relative z-10">Safe distribution</span>
                </div>
            </div>
