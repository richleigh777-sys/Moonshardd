...
export const SystemTab: React.FC<SystemTabProps> = ({ config, onChange }) => {
    return (
        <section className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
            <SectionHeader icon={ShieldAlert} title="Core Infrastructure" sub="Critical System Controls" color="text-red-500" />
            ...
            <div className="mt-12 relative group">
                ...
                <div className="relative bg-[#0f0a0a] rounded-[1.5rem] p-8 border border-red-500/30 overflow-hidden">
                    {/* Hazard Stripes */}
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
                        <div className="flex items-center gap-6">
                            <div className="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center border border-red-500/20 shadow-[0_0_30px_rgba(239,68,68,0.2)] shrink-0">
                                <ServerCrash size={32} className="text-red-500 animate-pulse"/>
                            </div>
                            <div>
                                <h4 className="text-2xl font-black text-red-500 uppercase tracking-tighter flex items-center gap-2">
                                    Emergency Flush
                                </h4>
                                <p className="text-[10px] text-red-400/70 font-bold uppercase tracking-widest mt-1">
                                    Local Cache & Session Purge
                                </p>
                                <p className="text-[10px] text-zinc-500 mt-2 max-w-md leading-relaxed">
                        ...
                        <button 
                            className="h-16 px-10 bg-red-600 hover:bg-red-500 text-white rounded-xl text-xs font-black uppercase tracking-[0.25em] shadow-lg shadow-red-900/40 flex items-center gap-3 group transition-all hover:scale-[1.02] active:scale-95 border border-red-400/50"
                            onClick={() => {
                ...
                />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-6 bg-surface-main border border-border-subtle rounded-2xl space-y-3">
                            <h4 className="text-xs font-black uppercase text-text-primary">Cloud Sync Reset</h4>
                            <p className="text-[10px] font-medium text-text-muted">Purge local cache and re-sync all operatives from Firestore.</p>
                            <Button 
                                variant="secondary" 
                                className="w-full h-10 gap-2"
                                onClick={() => window.location.reload()}
                            >
                                <RefreshCw size={14} /> Soft Reboot
                            </Button>
                        </div>
                        <div className="p-6 bg-surface-main border border-border-subtle rounded-2xl space-y-3">
                            <h4 className="text-xs font-black uppercase text-text-primary">Infrastructure Seeding</h4>
                            <p className="text-[10px] font-medium text-text-muted">Populate Firestore with default server and operative nodes.</p>
                            <Button 
                                variant="secondary" 
                                className="w-full h-10 gap-2 text-red-500 hover:bg-red-500/10"
                                onClick={async () => {
                            ...
                            >
                                <Database size={14} /> Hard Seed Cloud
                            </Button>
                        </div>
                    </div>
                </div>
