...
export const GlobalPerformanceSummary: React.FC<GlobalPerformanceSummaryProps> = ({ sales, users }) => {
    ...
    return (
        <Card variant="panel" className="p-6 lg:p-8 bg-surface-main/30 backdrop-blur-3xl shadow-panel border border-border-subtle relative overflow-hidden group hover:border-border-subtle transition-all rounded-3xl min-h-[160px] flex flex-col justify-center">
            ...
            <div className="flex items-center gap-5 mb-8 relative z-10">
                <div className="p-4 bg-gradient-to-br from-surface-highlight to-surface-main border border-border-subtle rounded-2xl text-accent-secondary group-hover:scale-110 transition-transform shadow-lg">
                    <BarChart3 size={24} strokeWidth={2} className="group-hover:animate-pulse drop-shadow-[0_0_12px_rgba(129,140,248,0.5)]" />
                </div>
                <div>
                    <h3 className="text-xs font-[700]  tracking-[0.2em] text-text-primary">Global Performance</h3>
                    <p className="text-[10px] font-bold text-text-muted  tracking-widest mt-1 opacity-80">Aggregate Intelligence Overview</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 relative z-10 w-full">
                <div className="space-y-4 p-5 rounded-3xl bg-surface-main/40 border border-border-subtle shadow-inner relative overflow-hidden group/item hover:bg-surface-main/60 transition-colors">
                    ...
                    <div className="flex items-center gap-3 text-text-secondary relative z-10">
                        ...
                        <span className="text-[10px] font-[700]  tracking-[0.2em]">Avg. Deal Size</span>
                    </div>
                    <div className="flex items-baseline gap-3 relative z-10">
                        <span className="text-4xl font-display font-[700] text-text-primary tracking-tighter">${Math.round(stats.avgDeal).toLocaleString()}</span>
                        <span className="text-xs font-[700] text-accent-secondary bg-accent-secondary/10 px-2.5 py-1 rounded-lg border border-accent-secondary/20">+5.2%</span>
                    </div>
                ...
                <div className="space-y-4 p-5 rounded-3xl bg-surface-main/40 border border-border-subtle shadow-inner relative overflow-hidden group/item hover:bg-surface-main/60 transition-colors">
                    ...
                    <div className="flex items-center gap-3 text-text-secondary relative z-10">
                        ...
                        <span className="text-[10px] font-[700]  tracking-[0.2em]">Rev. Per Agent</span>
                    </div>
                    <div className="flex items-baseline gap-3 relative z-10">
                        <span className="text-4xl font-display font-[700] text-text-primary tracking-tighter">${Math.round(stats.revPerAgent).toLocaleString()}</span>
                        <span className="text-xs font-[700] text-status-success bg-status-success/10 px-2.5 py-1 rounded-lg border border-status-success/20">+12.8%</span>
                    </div>
                    <div className="h-1.5 w-full bg-surface-alt/50 rounded-full overflow-hidden relative z-10 border border-border-subtle">
                ...
                <div className="space-y-4 p-5 rounded-3xl bg-surface-main/40 border border-border-subtle shadow-inner relative overflow-hidden group/item hover:bg-surface-main/60 transition-colors">
                    ...
                    <div className="flex items-center gap-3 text-text-secondary relative z-10">
                        ...
                        <span className="text-[10px] font-[700]  tracking-[0.2em]">Quota Attainment</span>
                    </div>
                    <div className="flex items-baseline gap-3 relative z-10">
                        <span className="text-4xl font-display font-[700] text-text-primary tracking-tighter">94.2%</span>
                        <span className="text-xs font-[700] text-status-warning bg-status-warning/10 px-2.5 py-1 rounded-lg border border-status-warning/20 shadow-[0_0_10px_var(--color-status-warning)] animate-pulse">On Track</span>
                    </div>
                    <div className="h-1.5 w-full bg-surface-alt/50 rounded-full overflow-hidden relative z-10 border border-border-subtle">
