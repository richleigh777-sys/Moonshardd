...
export const VisualAutomationBuilder = () => {
        ...
        return (
            <div className="flex flex-col items-center">
                <div 
                    ...
                    className={`relative w-72 rounded-2xl border p-4 cursor-pointer transition-all duration-300 group ${isSelected ? 'bg-surface-main border-accent-primary shadow-lg shadow-accent-primary/20 ring-1 ring-accent-primary/50 z-10' : 'bg-surface-main/60 border-border-subtle hover:bg-surface-main hover:border-accent-primary/50 shadow-sm'}`}
                >
                    {isRoot && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-accent-primary text-white text-xs font-[700]  tracking-widest rounded-full shadow-md flex items-center gap-1">
    ...
    return (
        <div className="h-full flex flex-col bg-surface-main relative overflow-hidden">
            <div className="h-16 px-6 border-b border-border-subtle flex items-center justify-between shrink-0 bg-surface-main/80 backdrop-blur-md z-20 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                    <button className="px-4 py-2 bg-surface-alt border border-border-subtle hover:bg-surface-main hover:text-text-primary text-text-muted rounded-xl text-xs font-bold  tracking-wider transition-all">
                        Test Workflow
                    </button>
                    <button 
                        onClick={handleSave}
                        className="px-6 py-2 bg-text-primary hover:bg-text-secondary text-surface-main rounded-xl text-xs font-bold  tracking-widest shadow-lg transition-all flex items-center gap-2"
                    >
                    ...
                    <div className="w-80 border-l border-border-subtle bg-surface-main/95 backdrop-blur-xl shrink-0 flex flex-col shadow-2xl z-20 animate-in slide-in-from-right-4 duration-300">
                        <div className="p-4 border-b border-border-subtle flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-xs font-[700]  tracking-widest text-text-secondary">Node Configuration</h3>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-6">
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-text-muted  tracking-wider">Node Name</label>
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-text-muted  tracking-wider">Node Type</label>
                                <select 
                            ...
                            {activeNode.type === 'action' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-text-muted  tracking-wider">Integration</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            <button className="flex flex-col items-center gap-2 p-3 rounded-xl border border-accent-primary bg-accent-primary/10 text-accent-primary shadow-sm hover:bg-accent-primary/20 transition-all">
                                                ...
                                                <span className="text-xs font-bold  tracking-wider">Twilio SMS</span>
                                            </button>
                                            <button className="flex flex-col items-center gap-2 p-3 rounded-xl border border-border-subtle bg-surface-alt text-text-muted shadow-sm hover:bg-surface-main hover:border-text-primary transition-all">
                                                <Webhook size={18} />
                                                <span className="text-xs font-bold  tracking-wider">Webhook</span>
                                            </button>
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-text-muted  tracking-wider">SMS Template</label>
                            ...
                            {activeNode.type === 'delay' && (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-text-muted  tracking-wider">Wait Amount</label>
                        ...
                        <div className="p-4 border-t border-border-subtle bg-surface-alt/50">
                            <button 
                                ...
                                className="w-full h-10 rounded-xl border border-status-error/30 text-status-error hover:bg-status-error hover:text-white transition-all text-xs font-bold  tracking-wider flex items-center justify-center gap-2"
                            >
