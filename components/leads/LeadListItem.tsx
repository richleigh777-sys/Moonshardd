...
export const LeadListItem: React.FC<LeadListItemProps> = ({ lead, isSelected, isOverdue, now, onSelect }) => {
    ...
    return (
        ...
        >
            ...
            {hasReminder && (
                <div className={`absolute right-0 top-0 p-1.5 ${isReminderOverdue ? 'text-status-error animate-pulse' : 'text-amber-500'}`}>
            ...
            <div className="flex justify-between items-start mb-1 pl-2">
                ...
                <span className={`text-[9px] font-mono font-bold ${isOverdue ? 'text-status-error animate-pulse' : 'text-text-muted'}`}>
            ...
            <div className="flex items-center gap-2 pl-2">
                ...
                <span className="text-[9px] text-text-muted truncate max-w-[120px]">{lead.reason}</span>
            </div>
        </div>
    );
};
