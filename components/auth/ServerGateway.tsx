...
export const ServerGateway: React.FC = () => {
    ...
    const getRegionColor = (region: string) => {
        if (region.includes('East')) return 'text-indigo-400';
        if (region.includes('West')) return 'text-amber-400';
        if (region.includes('EU')) return 'text-blue-400';
        return 'text-emerald-400';
    };

    return (
        <div className="h-screen w-full bg-[#050505] text-white flex flex-col items-center justify-center p-8 relative overflow-hidden font-sans">
            {/* Background Effects */}
            ...
            <div className="max-w-7xl w-full relative z-10 space-y-12 h-full flex flex-col">
                ...
                {/* Header */}
                <div className="flex flex-col md:flex-row justify-between items-end border-b border-white/10 pb-8 gap-6 shrink-0 mt-8">
                    <div>
                        <h1 className="text-5xl font-black uppercase tracking-tighter mb-4 flex items-center gap-4">
                            <div className="p-3 bg-indigo-600/20 rounded-2xl border border-indigo-500/30 text-indigo-400 shadow-neon">
                                <Database size={40} strokeWidth={1.5}/> 
                            </div>
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">Command Deck</span>
                        </h1>
                        <div className="flex items-center gap-4 text-xs font-mono text-slate-500 uppercase tracking-widest pl-2">
                            <span className="flex items-center gap-2"><Cpu size={12} className="text-emerald-500"/> System Online</span>
                            <div className="w-1 h-1 bg-slate-700 rounded-full"></div>
                            <span>Director Uplink • {currentUser?.name}</span>
                            <div className="w-1 h-1 bg-slate-700 rounded-full"></div>
                            <span className="text-amber-500 flex items-center gap-1"><Shield size={10}/> Security Level 10</span>
                        </div>
                    </div>
                    <Button onClick={openCreateModal} className="h-14 px-8 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20 rounded-2xl transition-all hover:scale-105 active:scale-95 flex items-center gap-3">
                    ...
                    {isPurging ? (
                        ...
                        <div className="flex flex-col h-[320px]">
                            <div className="flex items-center gap-3 text-red-500 mb-4 border-b border-red-900/30 pb-4">
                                ...
                                <h2 className="text-sm font-black uppercase tracking-[0.2em] animate-pulse">Destruction In Progress</h2>
                            </div>
                            <div className="flex-1 bg-black/50 rounded-xl p-4 font-mono text-[10px] text-red-500/80 overflow-y-auto custom-scrollbar space-y-1.5 border border-red-900/20 shadow-inner">
                                {purgeLogs.map((log, i) => (
                        ...
                        <>
                            <div className="flex items-center gap-4 text-red-500 mb-6">
                                ...
                                <div>
                                    <h2 className="text-xl font-black uppercase tracking-widest">Nuclear Protocol</h2>
                                    <p className="text-[10px] font-bold text-red-400/60 uppercase tracking-[0.2em]">Irreversible Action</p>
                                </div>
                            </div>
                            
                            <p className="text-sm font-medium text-slate-300 mb-6 leading-relaxed">
                                You are about to purge <span className="text-white font-black">{targetServer?.name}</span>. 
                                This will delete all sales data, user accounts, and audit logs associated with this node. 
                                This cannot be undone.
                            </p>

                            <div className="space-y-2 mb-8">
                                <label className="text-[10px] font-black uppercase text-red-500/60 tracking-widest ml-1">Type server name to confirm</label>
                                <input 
                            ...
                            <div className="flex gap-3">
                                <button 
                                    ...
                                    className="flex-1 h-14 rounded-xl border border-white/10 text-slate-400 font-bold uppercase tracking-wider text-xs hover:bg-white/5 transition-all"
                                >
                                    Abort
                                </button>
                                <button 
                                    ...
                                    className="flex-1 h-14 rounded-xl bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black uppercase tracking-widest text-xs shadow-lg shadow-red-900/40 flex items-center justify-center gap-2 group transition-all"
                                >
