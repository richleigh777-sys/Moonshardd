...
const pbe = `import React from 'react';
      ...
      {cart.length > 0 && (
        <div className="flex justify-between items-center bg-emerald-500/10 border border-emerald-500/20 rounded-2xl px-6 py-4 shrink-0 transition-all shadow-sm">
            ...
            <div className="text-right flex items-center gap-4">
                <span className="text-xs font-semibold text-emerald-500/70 tracking-wide uppercase">Subtotal</span>
      ...
      <div className="shrink-0 flex flex-col gap-4">
          ...
          <div className="p-5 bg-surface-alt/50 border border-white/5 rounded-[24px] shadow-sm flex flex-col gap-3">
             <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-white tracking-wide flex items-center gap-2 uppercase">
...
fs.writeFileSync('./components/forms/enrollment/v1/ProductBasketEnhanced.tsx', pbe);
